...
# @see https://github.com/jeremyevans/sequel/blob/5.71.0/lib/sequel/extensions/pg_array_ops.rb
#
...
module Sequel
  module Postgres
    ...
    class ArrayOp < Sequel::SQL::Wrapper
      ...
      def rs_to_string(joiner="", null=nil)
        query = <<~SQL.squish
        (
          SELECT
            listagg(json_serialize(el), ?)
          FROM
            ? AS el
        )
        SQL

        Sequel.lit(query, joiner, pg_array)
      end
      alias rs_join rs_to_string
